...
function formatOnSave(event) {
  ...
  if (importPattern) {
    ...
    switch (languageId) {
      case "python":
        importGroups = pythonFormatter.getPythonImportGroups(
          document,
          importPattern,
        );
        ...
        for (let i = 0; i < importGroups.length; i++) {
          ...
          if (importGroup.imports.length > 1) {
            if (importGroup.type === "singleline") {
              ...
              pythonFormatter.replaceMultiLineImportGroup(
                edit,
                importGroup,
                uri,
              );
        ...
        for (let i = 0; i < importGroups.length; i++) {
          ...
          if (importGroup.imports.length > 1) {
            if (importGroup.type === "singleline") {
              ...
              javascriptFormatter.replaceMultilineImportGroup(
                edit,
                importGroup,
                uri,
              );
...
function replaceSingleLineImportGroup(
  edit,
  importGroup,
  document,
  documentUri,
