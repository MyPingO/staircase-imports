...
function extractJavascriptImportGroups(lines) {
  ...
  if (currentGroup.length) {
    importGroups.push({
      type: multilineImport ? "multiline" : "singleline",
...
function replaceMultilineImportGroup(edit, importGroup, documentUri) {
  ...
  let sortedMultilineImports =
    startLine +
    sortedGroupLines.map((importLine) => `${importLine.line}`).join("\n") +
    "\n" +
    endLine;

  const endLineLength =
    vscode.window.activeTextEditor.document.lineAt(endLineIndex).text.length;
