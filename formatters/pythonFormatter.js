    ...
    if (isMultilineImport) {
      ...
      if (match) {
        ...
        importGroups.push({
          ...
          openingLineIndex: openingLineIndex,
          closingLineIndex: closingLineIndex,
        });
        currentGroup = [];
      } else {
        const message =
          "Failed to match multiline import. Please report this issue on the extension GitHub page!";
        const buttonLabel = "Report Issue";
        vscode.window
          .showErrorMessage(message, buttonLabel)
          .then((selection) => {
            if (selection === buttonLabel) {
              vscode.env.openExternal(
                vscode.Uri.parse(
                  "https://github.com/MyPingO/staircase-imports/issues",
                ),
              );
            }
          });
...
function replaceMultiLineImportGroup(edit, importGroup, documentUri) {
  ...
  const config = vscode.workspace.getConfiguration(
    "Staircase Import Formatter",
  );
  const sortOrder = config.get("order", "ascending");

  let sortedGroupLines = importGroup.imports.map(
    (importGroup) => importGroup.line,
  );
  sortedGroupLines.sort((lineA, lineB) =>
    sortOrder === "ascending"
      ? lineA.length - lineB.length
      : lineB.length - lineA.length,
  );
  ...
  sortedMultilineImports =
    ...
    "\n" +
    sortedGroupLines
      .map((line) => openingLeadingWhitespace + "\t" + line)
      .join("\n") +
    "\n" +
    closing;
